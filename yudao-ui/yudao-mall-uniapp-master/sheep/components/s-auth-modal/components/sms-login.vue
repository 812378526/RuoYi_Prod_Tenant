...
<script setup>
  ...
  async function smsLoginSubmit() {
    ...
    // 提交数据
    try {
      const res = await AuthUtil.smsLogin(state.model);
      if (!res || res.code !== 0) {
        console.error('[smsLogin] 登录失败，响应：', res);
        // 如果响应拦截器未展示错误（如返回 false 的场景），则手动展示提示
        if (!res) {
          sheep.$helper.toast('网络请求出错，请稍后重试');
        }
        return;
      }
      closeAuthModal();
    } catch (error) {
      console.error('[smsLogin] 登录异常：', error);
      sheep.$helper.toast(error?.msg || '登录失败，请稍后重试');
    }
  }
</script>
